<?php
    ...
    if (empty($id_connection_simrs_old)) {
    ...
    // =======================
    $QryCheck = $Conn->prepare("SELECT status_connection FROM connection_simrs_old WHERE id_connection_simrs_old = ?");
    if (!$QryCheck) {
    ...
    $QryCheck->bind_param("i", $id_connection_simrs_old);
    $QryCheck->execute();
    ...
    try {
        ...
        $QryDelete = $Conn->prepare("DELETE FROM connection_simrs_old WHERE id_connection_simrs_old = ?");
        if (!$QryDelete) {
            throw new Exception($Conn->error);
        }

        $QryDelete->bind_param("i", $id_connection_simrs_old);
