<?php
    ...
    if ($http_code !== 200) {
        echo '
            <div class="alert alert-danger">
                <small>Gagal mengambil data Imaging Study (HTTP ' . $http_code . ').</small>
            </div>
        ';
        exit;
    }
    ...
    if (!$data ||!is_array($data) ||isset($data['issue'])) {
        echo '
            <div class="alert alert-danger">
                <small>Data Imaging Study tidak valid atau tidak ditemukan.</small>
            </div>
        ';
        exit;
    }
    ...
    // Buat Variabelnya
    // Anggap $response adalah JSON string dari API
$data = json_decode($response, true);
