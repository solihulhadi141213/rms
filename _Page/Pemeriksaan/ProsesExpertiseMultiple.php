<?php
    ...
    $expertise_temuan  = $_POST['expertise_temuan'] ?? '';
    $expertise_kesan   = $_POST['expertise_kesan'] ?? '';
    $expertise_saran   = $_POST['expertise_saran'] ?? '';
    $expertise_catatan = $_POST['expertise_catatan'] ?? '';

    // ==============================================
    // CEK DATA SUDAH ADA ATAU BELUM
    // ==============================================
    $QryCheck = $Conn->prepare("SELECT id_radiologi_local_exp FROM radiologi_local_exp WHERE id_radiologi = ?");
    if (!$QryCheck) {
    ...
    if(empty($Data['id_radiologi_local_exp'])){
        ...
        // Proses Update
        $update = $Conn->prepare("UPDATE radiologi_local_exp SET temuan = ?, kesan = ?, saran = ?, catatan = ? WHERE id_radiologi_local_exp = ?");
        $update->bind_param("ssssi", $expertise_temuan, $expertise_kesan, $expertise_saran, $expertise_catatan, $id_radiologi_local_exp);
